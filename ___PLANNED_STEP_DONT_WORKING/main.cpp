...
struct T_FILTER_STRUCT {
    inline bool operator()(const T_STRUCT &s1, const T_STRUCT &s2) {
        return (s1.solutionStr != s2.solutionStr);
    }
};
...
std::ofstream fileSolutionsPrimes;
...
void writeDataToFile(T_STRUCT arr) {
    ...
    fileData << "___one___\n";
    fileData << (int) arr.one << "\n";
    fileData << (isPrime(arr.one) ? "true" : "false") << "\n";
    fileData << "__two__\n";
    fileData << (int) arr.two << "\n";
    fileData << (isPrime(arr.two) ? "true" : "false") << "\n";
    fileData << "__one__\n";
    fileData << (int) arr.one + sum << "\n";
    fileData << (isPrime(arr.one + sum) ? "true" : "false") << "\n";
    fileData << "___sum_plus_two___\n";
    fileData << (int) arr.two + sum << "\n";
    fileData << (isPrime(arr.two + sum) ? "true" : "false");
...
int main() {
    ...
    std::sort(arr.begin(), arr.end(), T_SORT_STRUCT_SUM());
    std::cout << arr.size() << std::endl;

    T_NUM sumPrimesSum = 0;
    T_NUM sumPrimesSolution = 0;
    T_NUM sumNotPrimesSum = 0;
    T_NUM sumNotPrimesSolution = 0;

    std::vector<T_PRIMES_VEC> solutions;

    for (T_NUM i = 0; i < arr.size(); i += 1) {
        T_STRUCT element = arr[i];

        for (unsigned long j = 0; j < element.comparisonPairs.size(); j++) {
            T_STR solutionStr = "";
            solutionStr = std::to_string(element.sum).append(
                    element.comparisonPairs.at(j) > 0 ? "+" : "").append(
                    std::to_string(element.comparisonPairs.at(j)));

            T_PRIMES_VEC solution;

            element.solution = element.sum + element.comparisonPairs.at(j);

            if (element.sum % 2 == 0 && element.comparisonPairs.at(j) % 2 == 1) {
                if (isPrime(element.sum)) {
                    fileSolutionsPrimes << "SUM IS PRIME" << std::endl;
                    sumPrimesSum++;
                } else {
                    sumNotPrimesSum++;
                }

                if (isPrime(element.solution)) {
                    fileSolutionsPrimes << "SOLUTION IS PRIME" << std::endl;
                    sumPrimesSolution++;
                } else {
                    sumNotPrimesSolution++;
                }

                fileSolutionsPrimes << "SUM: " << element.sum << std::endl;
                fileSolutionsPrimes << "SOLUTION: " << element.solution << std::endl;
                fileSolutionsPrimes << "EQUASION: " << solutionStr << std::endl << std::endl;

            } else if (element.sum % 2 == 1 && element.comparisonPairs.at(j) % 2 == 0) {
                if (isPrime(element.sum)) {
                    fileSolutionsPrimes << "SUM IS PRIME" << std::endl;
                    sumPrimesSum++;
                } else {
            ...
            solution.equation = solutionStr;
            solution.angle = element.angle;
            solution.result = element.sum + element.comparisonPairs.at(j);
            solution.sum = element.sum;

            if (!solutions.empty()) {
                if (solutions.at(solutions.size() - 1).equation != element.solutionStr) {
    ...
    fileSolutionsPrimes << "\n\n\n";
    fileSolutionsPrimes << "total numbers: " << solutions.size() << "\n";
    fileSolutionsPrimes << "sum - primes: " << sumPrimesSum << "\n";
    fileSolutionsPrimes << "sum - not primes: " << sumNotPrimesSum << "\n";
    fileSolutionsPrimes << "solution - primes: " << sumPrimesSolution << "\n";
    fileSolutionsPrimes << "solution - not primes: " << sumNotPrimesSolution << "\n";

    fileSolutionsPrimes.close();
